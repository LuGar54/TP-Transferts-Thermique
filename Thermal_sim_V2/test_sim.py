...
def get_harmonic_sol_temp(t_hours, params):
    """Température du sol (sinusoïdale)"""
    p = params["temperature_sol"]
    omega = 2 * np.pi / 24.0 # Cycle journalier supposé ou saisonnier selon A/phi
    # T(t) = T_mean + A * sin(...)
    return p["T_mean"] + p["A"] * np.sin(omega * (t_hours - p["t0"]))
...
def compute_Q_flow_net(i, T, flows, cp):
    """
...
def simulate_thermal_system(csv_path, t_end_hours=200):
    # 1. Chargement des données externes
    f_Text, duration_hours = load_external_data(csv_path)
    if f_Text is None:
        return
    ...
    T = np.ones(6) * 15.0 # Température initiale arbitraire (15°C)
    ...
    for k in range(steps):
        ...
        try:
            val_Text = float(f_Text(t_hour))
        except:
            val_Text = -5.0 # Fallback
            
        # Température sol (harmonique)
        val_Tsol = get_harmonic_sol_temp(t_hour, PARAMS)
        
        # --- B. Logique de Contrôle ---
        T_sensor = (T[0], T[5])
        
        # Règles :
        # 1. Arrêt forcé si T_ext > 3°C
        # 2. Allumage si T_sensor < 3°C
        # 3. Hystérésis : Arrêt si T_sensor > 30°C (pour éviter cycles courts)
        
        if val_Text > -0.2:
            heaters_state = False
        elif min(T_sensor) < 15.0:
            heaters_state = True
        elif max(T_sensor) > 45.0 and heaters_state:
            heaters_state = False
            
        # Sélection des débits et puissances
        if heaters_state:
            flows = PARAMS["flow_heaterON"]
            Q_aero = P_heater # Vecteur puissance
        else:
        ...
        if heaters_state:
            total_energy_kwh += (np.sum(P_heater) * dt) / (3.6e6)
        ...
        for i in range(6):
            ...
            # Flux = (Text - Ti) / R
            Q_ext = (val_Text - T[i]) / R_ext[i]
            
            # 2. Pertes vers le sol
            Q_sol = (val_Tsol - T[i]) / R_sol[i]
            
            # 3. Infiltration (Air extérieur entrant)
            # Q = m * cp * (Text - Ti)
            Q_inf = m_inf[i] * prop["cp_air"] * (val_Text - T[i])
            
            # 4. Échanges entre zones (Advection)
            Q_flow = compute_Q_flow_net(i, T, flows, prop["cp_air"])
...
if __name__ == "__main__":
    ...
    simulate_thermal_system(filepath, 300)